...
export default function PurchaseItems({
  ...
  async function handleStatusChange(
    ...
    if (currentStatus === "pending") {
      // Show price prompt before marking as purchased
  ...
  const totalEstimated = items.reduce(
    (sum, i) => sum + (i.estimated_price || 0) * (i.quantity || 1),
    0
  );
  const totalActual = items.reduce(
    (sum, i) =>
      sum +
      (i.actual_price || i.estimated_price || 0) * (i.quantity || 1),
    0
  );
    ...
    <div>
      ...
      {items.length > 0 && (
        <div className="mb-4 flex items-center gap-4 text-sm text-zinc-500 dark:text-zinc-400">
          <span>
            {items.filter((i) => i.status === "pending").length} pendiente(s)
          </span>
          <span>
            {items.filter((i) => i.status === "purchased").length} comprado(s)
          </span>
          <span>
            {items.filter((i) => i.status === "received").length} recibido(s)
          </span>
          {totalEstimated > 0 && (
            <span className="ml-auto">
              Est: {totalEstimated.toFixed(2)}&euro;
              {totalActual !== totalEstimated && (
                <> / Real: {totalActual.toFixed(2)}&euro;</>
              )}
            </span>
          )}
        </div>
        ...
        <div className="mb-4">
          {showForm ? (
            <form
              ...
              className="space-y-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-800/50"
            >
              <input
                type="hidden"
                      ...
                      {pricePromptId === item.id ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-24 rounded border border-zinc-300 px-2 py-1 text-xs dark:border-zinc-600 dark:bg-zinc-800 dark:text-white"
                          />
                          <button
                            type="button"
